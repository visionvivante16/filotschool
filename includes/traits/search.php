...
trait SearchTrait
{
  ...
  public function search_quick($query) {
    ...
    /* helper function to decode DB values */
    function normalize_search($value) {
        if (empty($value)) return '';
        $decoded = html_entity_decode($value, ENT_QUOTES | ENT_HTML5, 'UTF-8');
        $decoded = iconv('UTF-8', 'ASCII//TRANSLIT', $decoded);
        return strtolower(trim(preg_replace('/\s+/', ' ', $decoded)));
    }

    /* helper: ensure safe picture URL */
    function get_safe_picture_url($raw_path, $type) {
        $url = get_picture($raw_path, $type);
        ...
        if ($host !== $currentHost) {
            ...
            if ($headers && isset($headers[0]) && (strpos($headers[0], '200') !== false || strpos($headers[0], '304') !== false)) {
                return $url;
            }
        }

        $fallback = get_picture('', $type);
    ...
    function match_all_words($text, $words) {
        foreach ($words as $w) {
            if ($w == '') continue;
    ...
    /* search users */
    $get_users = $db->query("
        SELECT user_id, user_name, user_firstname, user_lastname, user_gender, user_picture, user_subscribed, user_verified
        FROM users
        WHERE user_banned = '0'
        LIMIT {$limit}
    ");
    ...
    while ($user = $get_users->fetch_assoc()) {
        $full   = normalize_search($user['user_firstname'] . ' ' . $user['user_lastname']);
        $first  = normalize_search($user['user_firstname']);
        $last   = normalize_search($user['user_lastname']);
        $uname  = normalize_search($user['user_name']);

        if (
            match_all_words($full, $queryWords) ||
            match_all_words($first, $queryWords) ||
            match_all_words($last, $queryWords) ||
            match_all_words($uname, $queryWords)
        ) {
            $user['user_picture'] = get_safe_picture_url($user['user_picture'], $user['user_gender']);
            $user['connection']   = $this->connection($user['user_id']);
            $user['sort']         = ($system['show_usernames_enabled']) ? $user['user_name'] : $user['user_firstname'];
            $user['type']         = 'user';
            $results[]            = $user;
    ...
    /* search pages */
    if ($system['pages_enabled']) {
        $get_pages = $db->query("SELECT * FROM pages LIMIT {$limit}");
        while ($page = $get_pages->fetch_assoc()) {
            $title = normalize_search($page['page_title']);
            $name  = normalize_search($page['page_name']);

            if (match_all_words($title, $queryWords) || match_all_words($name, $queryWords)) {
                $page['page_picture'] = get_safe_picture_url($page['page_picture'], 'page');
                $page['i_like']       = $this->check_page_membership($this->_data['user_id'], $page['page_id']);
                $page['sort']         = $page['page_title'];
                $page['type']         = 'page';
                $results[]            = $page;
    ...
    if ($system['groups_enabled']) {
        ...
        ");
        while ($group = $get_groups->fetch_assoc()) {
            $title = normalize_search($group['group_title']);
            $name  = normalize_search($group['group_name']);

            if (match_all_words($title, $queryWords) || match_all_words($name, $queryWords)) {
                $group['group_picture'] = get_safe_picture_url($group['group_picture'], 'group');
                $group['i_joined']      = $this->check_group_membership($this->_data['user_id'], $group['group_id']);
                $group['sort']          = $group['group_title'];
                $group['type']          = 'group';
                $results[]              = $group;
    ...
    if ($system['events_enabled']) {
        $get_events = $db->query("
            SELECT * FROM `events` 
            WHERE event_privacy != 'secret'
            LIMIT {$limit}
        ");
        while ($event = $get_events->fetch_assoc()) {
            $title = normalize_search($event['event_title']);
            if (match_all_words($title, $queryWords)) {
                $event['event_picture'] = get_safe_picture_url($event['event_cover'], 'event');
                $event['i_joined']      = $this->check_event_membership($this->_data['user_id'], $event['event_id']);
                $event['sort']          = $event['event_title'];
                $event['type']          = 'event';
                $results[]              = $event;
