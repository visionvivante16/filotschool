...
function mpesa_payment($amount, $phone, $reference) {
  global $system;
  $api_key = $system['mpesa_api_key'];
  $public_key = $system['mpesa_public_key'];
  $service_provider_code = $system['mpesa_service_provider_code'];
  $env = $system['mpesa_env'];
  ...
  $session_key = generateSessionKey($api_key, $public_key);
  ...
  $headers = [
      ...
      "Content-Type: application/json",
      // "Origin: " . $system['system_url'],
    "Origin: *",
      // "Referer: " . $system['system_url']
  ];
  $ch = curl_init($url);
  curl_setopt($ch, CURLOPT_POST, true);
  curl_setopt($ch, CURLOPT_HTTPHEADER, $headers);
  ...
  $response = json_decode($response, true);
  // print_r($response);
 file_put_contents(__DIR__.'/mpesa_debug.txt', print_r($response, true) . "\n", FILE_APPEND);
  if ($response === false) {
      return [
          'output_ResponseCode' => 'ERROR',
          'output_ResponseDesc' => curl_error($ch)
      ];
  }
  curl_close($ch);
  return $response;
...
function generateSessionKey($api_key, $public_key) {

  // ✅ FIX: add PEM format
  // $public_key = "-----BEGIN PUBLIC KEY-----\n" .
  //     chunk_split(trim($public_key), 64, "\n") .
  //     "-----END PUBLIC KEY-----";

  $public_key_decoded = base64_decode($public_key);
  $pem = der2pem($public_key_decoded);

  $new_key = openssl_pkey_get_public($pem);
  ...
  // openssl_public_encrypt($api_key, $encrypted, $pubKey);
  if (!openssl_public_encrypt($api_key, $encrypted, $new_key, OPENSSL_PKCS1_PADDING)) {
...
function convertUsdToMzn($usdAmount) {

  // 🔥 Free API (you can change provider)
  $url = "https://api.exchangerate-api.com/v4/latest/USD";

  $response = file_get_contents($url);
  $data = json_decode($response, true);
  if (!isset($data['rates']['MZN'])) {
      return false;
  }
