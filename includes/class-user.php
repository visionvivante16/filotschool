...
class User
{
  ...
  public function wallet_marketplace_mpesa_success($orders_collection_id, $ref)
  {
      global $db;
      ...
      if (!$orders_collection || $orders_collection['paid']) {
          return false;
      }
      ...
      // ✅ Loop orders → credit sellers
      $order = $orders_collection['orders'][0];

      $seller_id = $order['seller_id'];
      $order_id  = $order['order_id'];
      $amount    = $order['sub_total'];

      $commission_percent = $order['commission'];

      $commission =  round(($amount * $commission_percent) / 100, 2);
      $seller_earning = round($amount - $commission, 2);

      // seller wallet
      $db->query(sprintf("
          UPDATE users 
          SET user_wallet_balance = user_wallet_balance + %s 
          WHERE user_id = %s
      ", secure($seller_earning), secure($seller_id)));

      // seller transaction
      $this->wallet_set_transaction(
          $seller_id,
          'market_sale',
          $order_id,
          $seller_earning,
      ...
      $db->query(sprintf("
          ...
          SET user_wallet_balance = user_wallet_balance + %s 
          WHERE user_id = %s
      ", secure($commission), secure($admin_id)));
